import os,sys,threading,keyboard,pygame,pygame_gui,importlib;sys.dont_write_bytecode=True;os.environ['PYGAME_HIDE_SUPPORT_PROMPT']='1';from win32gui import GetWindowText, GetForegroundWindow;from pygame_gui.elements import UIPanel,UIButton;PANEL_LAYER = 10;BUTTON_DIMS = (200, 25);pygame.display.set_icon(pygame.image.load(os.path.join(os.path.abspath(os.path.dirname(__file__)),'image.jpg')))
class Menu(UIPanel):
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN and event.key == pygame.K_HOME and self.APPMENU is None:self.APPMENU = Menu(self.MANAGER,(0, 0),"Rosehip.apps",self.APPS,self.appmenu_load,)
...
def pc():OS().run()
def laptop():threading.Thread(target=switch).start();OS().run()
